...
class CharacterRecognition:
    ...
    def group_adjacent_characters(char_images_with_coordinates):
        ...
        for i in range(len(char_images_with_coordinates) - 1):
            ...
            if abs(current_char_info[1] - next_char_info[1]) < some_threshold:
    ...
    def organize_characters_in_pdf(self, output_pdf_path, char_groups, input_image_path):
        try:
            ...
            for group in char_groups:
                ...
                group.sort(key=lambda char: char[1])

                # Initialize variables to track adjacent characters
                prev_x, prev_y, prev_w, prev_h = group[0][1]
                adjacent_chars = [(group[0][0], group[0][2], group[0][3])]

                for char_info in group[1:]:
                    ...
                    if x - prev_x <= prev_w:
                        adjacent_chars.append((char_output_filename, predicted_char_index, y, h))
                    else:
                        ...
                        adjacent_chars = [(char_output_filename, predicted_char_index, y, h)]

                    # Update previous character information
    ...
    def draw_adjacent_characters(self, pdf, char_info_list, img_height):
        ...
        for char_info in char_info_list:
            char_output_filename, predicted_char_index, y, h = char_info

            # Extract color information based on your implementation
            color = char_info[3]

            # Convert hex_color to RGB
            ...
            # Pass x coordinate as a parameter
            x = char_info[2]
                ...
                for stroke_object in my_array:
                    ...
                    if self.is_point_inside_rectangle(point_to_check, rectangle_origin, rectangle_width,
                        ...
                        char_images_with_coordinates.append(
                            ((char_output_filename, (x_orig, y_orig, w, h)), predicted_char_index, pos.color))
    ...
    try:
        ...
        try:
            ...
            for slide_data in organized_data:
                ...
                char_recognizer.change_background_and_text_color(image_path, output_image_path, output_directory,
                                                                 my_array, slide_number, Slide_count)
                char_recognizer.save_to_pdf(output_pdf_path, char_images_with_coordinates, image_path, slide_number,
                                            Slide_count)
